...
var { Document, Footer, Header, Packer, Paragraph } = require('docx')
...
exports.Document = function () {
  ...
  this.insertDocxSync = function (docxData) {
    var xml = this.getExternalDocxRawXml(docxData)
    this.insertRaw(xml)
  }

  this.save = function (filename, err) {
    generateTemplate().then((buffer) => {
      ...
      if (this.rels.length > 0) {
        ...
        for (var i = 0; i < this.rels.length; i++) {
          ...
          if (rel.target != rel.newTarget) {
            ...
            } else filesToSave[saveTo] = this._utf8ArrayToString(rel.data)
